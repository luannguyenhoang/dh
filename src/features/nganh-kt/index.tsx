...
export const Kt = async () => {
  ...
  const credits = parseInt(
    nganhKeToan?.chuongTrinhVaThoiGianDaoTao?.label1?.text2 || defaultDataKt.credits.toString()
  );
  const subjects = parseInt(
    nganhKeToan?.chuongTrinhVaThoiGianDaoTao?.label2?.text2 || defaultDataKt.subjects.toString()
  );
  ...
  const notifyData = {
    ...
    noiDung:
      ktData?.tuyenSinh?.header?.text ||
      defaultDataKt.notifyData.noiDung,
    tuyenSinh: {
      label1: {
        child: ktData?.tuyenSinh?.label1?.child || defaultDataKt.notifyData.tuyenSinh.label1.child,
      },
      label2: {
        image: ktData?.tuyenSinh?.label2?.image || defaultDataKt.notifyData.tuyenSinh.label2.image,
      },
    },
  };
  ...
  return (
    ...
    >
      <Branch
        ...
        overview={
          nganhKeToan?.tongQuan?.label?.map((item: any) => item.text) || defaultDataKt.overview
        }
        jobs={
          nganhKeToan?.ngheNghiep?.label?.map((item: any) => item.text) || defaultDataKt.jobs
        }
        program={{
          credits,
          subjects,
          ...
          list: nganhKeToan?.chuongTrinhVaThoiGianDaoTao?.label2?.map(
            (item: any) => ({
              title: item.text1 || "",
              content: item.text2 || "",
            })
