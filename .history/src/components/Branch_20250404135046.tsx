...
export const Branch = (props: IBranch) => {
  const { name, overview, jobs, program, universityInfo } = props;
  return (
    <Box color={"blue.800"}>
      <Heading
        as={"h2"}
      ...
      >
        {universityInfo?.map((info, index) => (
          <HStack key={index}>
            <Icon as={index === 0 ? LiaUniversitySolid : index === 1 ? PiStudent : BsLaptop} />
            <Text>{info.text}</Text>
          </HStack>
        ))}
      </HStack>
      ...
      <Box pt={"32px"}>
        <HStack pb={"16px"}>
          ...
          <Heading as={"h2"} size={"md"}>
            {program?.tongQuan?.title || "Tổng quan"}
          </Heading>
        </HStack>
      ...
      <Box pt={"32px"}>
        <HStack pb={"16px"}>
          <Icon as={GrOverview} />
          <Heading as={"h2"} size={"md"}>
            {program?.ngheNghiep?.title || "Nghề nghiệp"}
          </Heading>
        </HStack>
      ...
      <Box pt={"32px"}>
        <HStack pb={"16px"}>
          <Icon as={GrOverview} />
          <Heading as={"h2"} size={"md"}>
            {program?.chuongTrinhVaThoiGianDaoTao?.title || "Chương trình và thời gian đào tạo"}
          </Heading>
        </HStack>
