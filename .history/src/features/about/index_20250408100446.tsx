    ...
    <Box color={"blue.800"}>
      ...
      <Container maxW={"6xl"} py="60px">
        <Heading as="h2" size={{ base: "md" }} py="16px" pb={"10px"}>
          {aboutData.title || "Lịch sử hình thành và phát triển Đại học Thái Nguyên"}
        </Heading>
        <Text>
          {aboutData.description || "Đại học Thái Nguyên (ĐHTN – tên tiếng Anh: ThaiNguyen University; viết tắt là TNU) được thành lập ngày 04/04/1994 theo Nghị định số 31/CP của Chính Phủ trên cơ sở tổ chức sắp xếp lại các trường đại học thuộc địa bàn tỉnh Thái Nguyên."}
        </Text>
        <Grid placeItems={"center"} py={"24px"}>
          <Image
            src={aboutData.anhGioiThieu?.node?.mediaItemUrl || "/truong-dai-hoc-nong-lam-thai-nguyen.jpg"}
        ...
        <Box>
          ...
          <Text fontWeight={"700"}>{aboutData.giaTriCotLoi?.text2 || "SÁNG TẠO - NHÂN VĂN - CHẤT LƯỢNG"}</Text>
        </Box>
        ...
        <Box>
          ...
          <Text fontWeight={"600"}>
            {aboutData.slogan?.text2 || "Cùng kiến tạo những giá trị mới - Together we create new success."}
          </Text>
        </Box>
