...
export const Cntt = async () => {
  ...
  const notifyData = {
    ...
    noiDung: cnttData?.tuyenSinh?.header?.text || "Thông báo tuyển sinh hệ từ xa Đại học Thái Nguyên 2023",
    cot: {
      cot1: cnttData?.tuyenSinh?.label1?.child?.map((item: any) => ({
        title: item.title,
        noiDung: item.text
      })) || [],
      anh: cnttData?.tuyenSinh?.label2?.image || { node: { mediaItemUrl: "/phoi-bang-dh-thai-nguyen.jpg" } }
    }
  };

  return (
    <LayoutNganh title={cnttData?.tieuDe || "Ngành công nghệ thông tin"} data={notifyData}>
      <Branch
