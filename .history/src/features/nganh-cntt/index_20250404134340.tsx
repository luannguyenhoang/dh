...
export const Cntt = async () => {
  ...
  // Extract credits and subjects from the API response
  const credits = parseInt(nganhHoc?.chuongTrinhVaThoiGianDaoTao?.label?.[0]?.cot?.text2 || "124");
  const subjects = parseInt(nganhHoc?.chuongTrinhVaThoiGianDaoTao?.label?.[1]?.cot?.text2 || "42");

  return (
    <LayoutNganh title={cnttData?.tieuDe || "Ngành công nghệ thông tin"}>
      <Branch
        name={nganhHoc?.title || "Công nghệ thông tin"}
        overview={nganhHoc?.tongQuan?.label?.map((item: any) => item.text) || [
        ...
        program={{
          ...
          list: nganhHoc?.chuongTrinhVaThoiGianDaoTao?.label2?.map((item: any) => ({
            title: item.text1 || "",
            content: item.text2 || "",
          })) || [
            {
