  ...
  try {
    ...
    const exactMatches = posts.filter((post: any) => {
      const title = (post.title?.toLowerCase() || "")
        ...
        .replace(/\s+/g, " ")
        .trim();
      console.log("Comparing title:", title);
      const isExact = title === normalizedSearch;
      if (isExact) {
        console.log("Found exact match:", post.title);
      }
      return isExact;
    });
...
async function getAllPosts(client: ApolloClient<any>) {
  let allPosts: any[] = [];
